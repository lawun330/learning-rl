...
class MonteCarloEngine:
    def __init__(self):
        ...
        self.returns = { (r, c): [] for r in range(ROWS) for c in range(COLS) }
        self.gamma = 0.9
    ...
    def generate_episode(self):
        ...
        while state not in [GOAL] + TRAPS:
            ...
            if (r, c) not in WALLS: state = (r, c)
            
            reward = 1.0 if state == GOAL else (-1.0 if state in TRAPS else -0.04)
            path.append((state, action, reward, state))
    ...
    def get_detailed_log(self):
        ...
        visited = set()
        for t, (state, action, reward, next_s) in enumerate(reversed(self.episode_history)):
            G_prev = G
            G = reward + (self.gamma * G_prev)
            log += f"Step {t+1}: State {state} | G = {reward:+.2f} + ({self.gamma} * {G_prev:+.4f}) = {G:+.4f}\n"
            if state not in visited:
                visited.add(state)
                self.returns[state].append(G)
                self.V[state] = sum(self.returns[state]) / len(self.returns[state])
                log += f"  >> V(s) updated to {self.V[state]:+.4f}\n"
            else:
                log += f"  >> SKIP (First-Visit MC rule)\n"
        return log

class MCVisualizer:
    def __init__(self, root):
        self.root = root
        self.root.title("Monte Carlo Learning Lab")
        ...
        ttk.Button(ctrl, text="2. Calculate & Learn", command=self.run_learn).pack(fill="x", pady=2)
        ...
        theory_text = (
            "MATH: G_t = R_t + (γ * G_{t+1})\n"
            "   - G: Return (Expected long-term reward)\n"
            "   - R: Immediate Reward (-0.04 step, -1 trap, +1 goal)\n"
            "   - γ: Discount (0.9, values future rewards less)\n\n"
            "COLORS:\n"
            "   - Green: High Value (Good Path)\n"
            "   - Red: Low Value (Near Trap/Dead End)\n"
            "   - White: Unexplored/Neutral"
        )
